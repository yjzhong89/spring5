...
class ConfigurationClassEnhancer {
	...
	private Enhancer newEnhancer(Class<?> configSuperClass, @Nullable ClassLoader classLoader) {
		...
		// 下面的EnhancedConfiguration接口继承了BeanFactoryAware接口
		// BeanFactoryAware接口中有一个setBeanFactory()方法
		// 所以当需要获取一个bean时，不会直接去new一个对象，而是看BeanFactory中有没有这个bean
		enhancer.setInterfaces(new Class<?>[] {EnhancedConfiguration.class});
		enhancer.setUseFactory(false);
		enhancer.setNamingPolicy(SpringNamingPolicy.INSTANCE);

		// BeanFactoryAwareGeneratorStrategy是一个类的生成策略
		// 主要为生成的CGLIB类中设置一个成员变量$$beanFactory
		// 同时基于接口EnhancedConfiguration的父接口BeanFactoryAware中的setBeanFactory()方法
		// 将此变量设置为当前context中的beanFactory，使得cglib的代理类中就有了beanFactory($$beanFactory)
		// 有了beanFactory就可以直接获取对象，而不需要通过方法去获得
		enhancer.setStrategy(new BeanFactoryAwareGeneratorStrategy(classLoader));
		// 对方法进行过程，不会每次都去new一个对象
	...
	private static class BeanMethodInterceptor implements MethodInterceptor, ConditionalCallback {
		...
		public Object intercept(Object enhancedConfigInstance, Method beanMethod, Object[] beanMethodArgs,
			...
			// 获取cglib生成的代理对象的成员变量$$beanFactory的值
			...
			// 判断执行的方法和调用的方法是不是同一个(即被代理的方法和正在执行的方法是不是同一个)
			// 判断是new一个对象还是直接getBean
