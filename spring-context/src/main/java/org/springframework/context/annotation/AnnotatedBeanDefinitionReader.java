...
public class AnnotatedBeanDefinitionReader {
	...
	private <T> void doRegisterBean(Class<T> annotatedClass, @Nullable String name,
		...
		// 处理类当中的通用注解
		// 通过源码分析可以知道这里主要处理lazy dependsOn primary role description等注解
		// 处理完成后，通过processCommonDefinitionAnnotations方法进行设置
		AnnotationConfigUtils.processCommonDefinitionAnnotations(abd);

		// 如果在向容器注册注解bean定义时，使用了额外的限定符注解则解析
		// qualifiers是Annotation类型的数组，里面存放的不仅仅是Qualifier的注解
		// 理论上里面存放的是一切注解
		// 但是在调用链上可以发现这里的qualifiers为null
		if (qualifiers != null) {
		...
		// 将上述的数据结构注册到registry
		// 其中registry就是AnnotationConfigApplicationContext
		// AnnotationConfigApplicationContext在初始化的时候通过调用父类的构造方法构造了DefaultListableBeanFactory
		// registerBeanDefinition方法将definitionHolder包含的信息注册到DefaultListableBeanFactory中
