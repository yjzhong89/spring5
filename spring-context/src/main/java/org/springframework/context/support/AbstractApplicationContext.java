...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			try {
				...
				// 设置执行自定义的以及spring内部的BeanFactoryPostProcessor
				// spring内部的只有一个，即ConfigurationClassPostProcessor
				// 这个类是在new AnnotatedBeanDefinitionReader(this)时添加的
				invokeBeanFactoryPostProcessors(beanFactory);

				// Register bean processors that intercept bean creation.
				// 注册bean的后置处理器来插手bean的实例化
				...
				// 在目标对象实例化之前调用，方法的返回值类型是Object，可以返回任何类型的值。
				...
				// 6. InstantiationAwareBeanPostProcessor#postProcessProperties方法
				// 7. BeanPostProcessor#postProcessBeforeInitialization方法
				// 8. BeanPostProcessor#postProcessAfterInitialization方法(初始化过程，当bean已经实例化，并且完成了属性的填充)
				// 9. bean销毁的后置处理器
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		/**
		 *  配置其标准特征，比如上下文的加载器ClassLoader和postprocessors的回调
		 *  该方法的参数是DefaultListableBeanFactory
		 */
		// Tell the internal bean factory to use the context's class loader etc.
		beanFactory.setBeanClassLoader(getClassLoader());
		// 添加bean表达式解析器
		// 让beanFactory可以去解析bean的表达式，即可以在前台页面获取bean当中的属性
		...
		// 就注册两个bean，key分别为systemProperties和systemEnvironment，value为map
		// 存放的是一些系统配置和系统环境信息
	...
	protected void invokeBeanFactoryPostProcessors(ConfigurableListableBeanFactory beanFactory) {
		...
		// (自定义是指没有交给spring去管理，需要手动去调用addBeanFactoryPostProcessor方法)
